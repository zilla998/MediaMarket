...
class CustomUser(AbstractUser):
    ...
    phone = models.CharField(
        validators=[phone_regex], max_length=255, verbose_name="Номер телефона"
    )
    avatar = models.ImageField(
        upload_to="users_avatar/", blank=True, null=True, verbose_name="Аватар"
    )
    birth_date = models.DateField(blank=True, null=True, verbose_name="День рождения")
    gender = models.CharField(
        choices=choices, blank=True, null=True, verbose_name="Гендер"
    )
    ...
    newsletter_subscribed = models.BooleanField(
        default=False, blank=True, null=True, verbose_name="Согласие на рассылку"
    )
