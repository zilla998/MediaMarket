...
class UserProfileSettingsForm(forms.ModelForm):
    ...
    password = forms.CharField(label="Пароль", widget=forms.PasswordInput(attrs={'class': 'form-input'}))
    password2 = forms.CharField(label="Повтор пароля", widget=forms.PasswordInput(attrs={'class': 'form-input'}))
    avatar = forms.ImageField(label="Аватар", widget=forms.FileInput(attrs={'class': 'form-input'}))
    birth_date = forms.DateField(label="Дата рождения", widget=forms.DateInput(attrs={'class': 'form-input'}))
    gender = forms.BooleanField(label="Пол", widget=forms.CheckboxInput(attrs={'class': 'form-input'}))
